...
import { Avatar, Chip, EmptyState, Segmented } from "@/components/ui";
...
export default function ExplorePage() {
  ...
  const cooks = useMemo(() => {
    ...
    return SEED_PROFILES.map((p) => ({ profile: p, recipes: counts.get(p.id) ?? 0 })).sort(
      (a, b) => b.profile.followers - a.profile.followers,
    );
  }, [state]);
          ...
          {cooks.map(({ profile, recipes }) => (
            ...
            >
              <div className="flex justify-center">
                <Avatar emoji={profile.emoji} gradient={profile.gradient} size={54} />
              </div>
